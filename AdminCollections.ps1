#############################################################################
# Frank Maxwitat based on the script of Benoit Lecours, SystemCenterDudes.com 
# First release 22.04.2023, v1.0.0
#            
# The script creates a set of SCCM collections and moves them into a "Bechtle Administration" folder
# The update interval is set to weekly on Sunday, the time will be the creation time of the collection
#
#############################################################################
...
$CollectionFolder = @{Name ="Administrative Collections"; ObjectType =5000; ParentContainerNodeId =0}
...
$AllDesktopsAndServers="All Desktop And Server Clients"

#Refresh Schedule
$Schedule =New-CMSchedule -DayOfWeek Sunday -Start  (Get-Date) #–RecurInterval Days –RecurCount 7
...
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | No cummulative update for more than 60 days"}},@{L="Query"
; E={"SELECT SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client FROM SMS_R_System JOIN SMS_UpdateComplianceStatus ON SMS_R_System.ResourceID = SMS_UpdateComplianceStatus.MachineID JOIN SMS_SoftwareUpdate ON SMS_UpdateComplianceStatus.CI_ID = SMS_SoftwareUpdate.CI_ID 
where SMS_UpdateComplianceStatus.Status = 2 AND SMS_SoftwareUpdate.LocalizedDisplayName Like '%cumulative update for windows 10%' AND dateDiff(day, SMS_SoftwareUpdate.DateRevised, GetDate()) > 60"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"No cummulative update for more than 60 days"}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | Low disk space: < 1GB"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System inner join SMS_G_System_LOGICAL_DISK on SMS_G_System_LOGICAL_DISK.ResourceID = SMS_R_System.ResourceId 
where SMS_G_System_LOGICAL_DISK.FreeSpace < 1024"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"All devices with less than 1 GB disk space"}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | Low disk space: < 5GB"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System inner join SMS_G_System_LOGICAL_DISK on SMS_G_System_LOGICAL_DISK.ResourceID = SMS_R_System.ResourceId 
where SMS_G_System_LOGICAL_DISK.FreeSpace < 5120"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"All devices with less than 5 GB disk space"}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | WUA service disabled"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System inner join SMS_G_System_SERVICE on SMS_G_System_SERVICE.ResourceID = SMS_R_System.ResourceId 
where SMS_R_System.Client = '1' and SMS_R_System.Obsolete = '0' and SMS_R_System.Active = '1' and SMS_G_System_SERVICE.Name = 'wuauserv' and SMS_G_System_SERVICE.StartMode = 'Disabled'"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"WUAU Service is set to disabled. Software updates cannot be installed."}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | WUA service not up-to-date"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System inner join SMS_G_System_WINDOWSUPDATEAGENTVERSION on SMS_G_System_WINDOWSUPDATEAGENTVERSION.ResourceID = SMS_R_System.ResourceId 
where SMS_R_System.Client = '1' and SMS_R_System.Obsolete = '0' and SMS_R_System.Active = '1' and SMS_G_System_WINDOWSUPDATEAGENTVERSION.Version is not null and SMS_G_System_WINDOWSUPDATEAGENTVERSION.TimeStamp < DateAdd(dd,-185,GetDate())"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"WUAU service has not been updated in over six months. If the servicing stack is too far behind it will not be able to apply updates."}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | Systems that are not in AD"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System 
where SMS_R_System.OperatingSystemNameandVersion like '%windows%' and (SMS_R_System.Obsolete != '0' or SMS_R_System.Obsolete is null)"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"These devices are no longer in Active Directory.  They may not exist on the network anymore."}}

$Collections +=
$DummyObject |
Select-Object @{L="Name"
; E={"System Health | No heartbeat since 30 days"}},@{L="Query"
; E={"select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System 
where AgentName in ('Heartbeat Discovery') and DATEDIFF(day,AgentTime,GetDate())<=30"}},@{L="LimitingCollection"
; E={$AllDesktopsAndServers}},@{L="Comment"
; E={"These clients have not sent a heartbeat in over 30 days"}}
...
$DummyObject |
Select-Object @{L="Name"
; E={"Systems | Created during last 24h"}},@{L="Query"
...
; E={"All systems that are SCCM site servers"}}
...
Else{
        Write-host "-----------------"
        Write-Host -ForegroundColor Green "Script execution completed without error. Bechtle Administration Collections created sucessfully."
        Write-host "-----------------"
    }
}
